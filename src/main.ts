...
const DAY_CYCLE = 30; // seconds for visual day/night cycle (no terrain regen)

// Grid shift parameters
const SHIFT_THRESHOLD = 40; // rows of buffer ahead of screen bottom
const SHIFT_AMOUNT = 30;    // rows to shift at a time
...
async function main() {
  ...
  // Create terrain grid and fill with initial procedural terrain
  const terrain = createTerrainGrid();
  generateRows(terrain, 0, GRID_H);
  ...
  function tick(input: InputState, dt: number) {
    ...
    // Camera follows player downward (one-way)
    updateCameraScroll(camera, player.y, gpu.canvas.height, dt);

    // Grid shifting: when buffer below the screen gets thin, shift up and generate new rows
  ...
  function loop(now: number) {
    ...
    // For player/projectile/particle: screenY = worldY - scrollY
    const sx = camera.shakeX;
    const sy = -camera.scrollY + camera.shakeY;

    // For terrain: account for grid buffer offset
    // gy_local = (screenPixelY - terrainCameraY) / CELL_SCALE
    // We need gy_local = worldGridY - worldYOffset
    // So terrainCameraY = -(scrollY - worldYOffset * CELL_SCALE)
    const terrainCameraX = camera.shakeX;
    const terrainCameraY = -camera.scrollY + terrain.worldYOffset * CELL_SCALE + camera.shakeY;

    // Upload only newly-emitted particle slots to GPU
    ...
    // Render projectile shapes with camera offset
